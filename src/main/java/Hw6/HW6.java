...
//    Отфильтровать ноутбуки их первоначального множества и вывести проходящие по условиям.
    ...
    private static void findParameter(Set<Notebooks> notebooksSet) {
        ...
        System.out.println("\nВведите номера критерий поиска через пробел: ");
        ...
        for (int i : criteria) {
            ...
            if (i <= 3) {
                ...
                System.out.printf("Введите минимальное требуемое значение %s ", name[i - 1]);
        ...
        for (Notebooks notebook : notebooksSet) {
            if (criteriaLetter.get(1).contains(notebook.getName()) && criteriaLetter.get(2).contains(notebook.getCpu())
                    && criteriaLetter.get(3).contains(notebook.getOs()) && criteriaDigit.get(4) <= notebook.getHdd()
                    && criteriaDigit.get(5) <= notebook.getRam() && criteriaDigit.get(6) <= notebook.getPrice()) {
